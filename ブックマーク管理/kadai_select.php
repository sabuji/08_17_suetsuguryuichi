...
if ($status==false) {
    ...
    while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $view .= '<p>'.$result['id'].'-'.$result['name'].'-'.$result['url'].'-'.$result['comment'].'-'.$result['indate'].'</p>';
...
<body>
    <header>
        <nav class="navbar navbar-expand-lg navbar-light bg-light">
            <a class="navbar-brand" href="#">ブックマークリスト一覧</a>
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav">
                    <li class="nav-item">
                        <a class="nav-link" href="kadai_index.php">ブックマークリスト登録</a>
                    </li>
                </ul>
            </div>
        </nav>
